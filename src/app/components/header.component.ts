    ...
    >
      <div class="p-4">
        ...
        <ul class="space-y-4">
          ...
          <li class="relative group">
            <a
              (click)="toggleMobileTeamsDropdown($event)"
              class="hover:text-[#D29C9C] cursor-pointer text-lg"
              >Xpress Teams</a
            >
            <ul
              [class.hidden]="!isMobileTeamsDropdownOpen"
              class="bg-[#161659] p-2 z-10"
            >
              <li *ngFor="let team of teams">
                <a
                  [routerLink]="['/team', team.id]"
...
export class HeaderComponent {
  ...
  isMobileTeamsDropdownOpen = false;
  ...
  closeMobileMenu() {
    ...
    this.isMobileTeamsDropdownOpen = false;
    const mobileMenu = document.querySelector('.mobile-menu');
    mobileMenu?.classList.remove('open');
  }

  toggleMobileTeamsDropdown(event: Event) {
    event.stopPropagation();
    this.isMobileTeamsDropdownOpen = !this.isMobileTeamsDropdownOpen;
  }
}
